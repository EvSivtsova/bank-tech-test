...
describe BankAccount do
  context 'when depositing money' do
    it 'records and returns the value and the date of the transactions' do
      ...
      expect(bank_account.transactions[0]).to include({ date: '10/01/2023', value: 1000, balance: 1000 })
    end
    ...
    it 'updates the balance of the account when several transaction have been made' do
      ...
      expect(bank_account.balance).to eq 3000
      expect(bank_account.transactions[1]).to include({ date: '22/08/2022', value: 2000, balance: 3000 })
    end
  end
  ...
  context 'when withdrawing money' do
    ...
    it 'updates the balance when several transaction have been made' do
      ...
      expect(bank_account.balance).to eq(-1500)
      expect(bank_account.transactions[1]).to include({ date: '23/08/2022', value: -500, balance: -1500 })
    end
  end
  ...
  context 'when both depositing and withdrawing money' do
    it 'records withdrawal and deposit transations' do
      ...
      expect(bank_account.transactions[0]).to include({ date: '22/08/2022', value: 1000, balance: 1000 })
      expect(bank_account.transactions[1]).to include({ date: '11/01/2023', value: -500, balance: 500 })
      expect(bank_account.transactions[2]).to include({ date: '12/01/2023', value: 3000, balance: 3500 })
      expect(bank_account.transactions[3]).to include({ date: '13/01/2023', value: -2000, balance: 1500 })
    end
  end
